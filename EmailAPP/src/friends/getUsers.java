    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            while(rs.next()) {
                ...
                String queryCheck2 = "select status from friends where friend_one like (select id from users where username=?) and friend_two like (select id from users where username=?)";
                ...
                if(!rscheck.next() || (!rscheck.getString(1).equals("1") && !rscheck.getString(1).equals("2")))
                {
                    out.println("<tr>");
                    out.println("<td class=\"FR\"><p style=\"padding: 10px 0px; margin-bottom: 0px;\">"+rs.getString(1)+"</p></td>");
                    out.println("<td>");
                    if(rscheck2.next() && rscheck2.getString(1).equals("1")) {
